...
from gan_models import Discriminator, DiscriminatorInd, Generator, ImageInpaintingModel
...
if __name__ == "__main__":
    ...
    train_df, test_df = pd.DataFrame(specdata["train_df"], columns=df_cols), pd.DataFrame(
        specdata["test_df"], columns=df_cols
    )
    ...
    if use_aind:
        train_loader = DataLoader(
            dataset=SpecDatasetImInd(train_df.iloc[:train_b], im_path, pop_classes, a_indices),
            batch_size=batch_size,
            shuffle=True,
        )
    else:
        train_loader = DataLoader(
            dataset=SpecDatasetIm(train_df.iloc[:train_b], im_path, pop_classes),
            batch_size=batch_size,
            shuffle=True,
        )
    # test_loader  = DataLoader(
    #    dataset=SpecDatasetIm(test_df.iloc, im_path, pop_classes),
    #    batch_size=batch_size
    # )
    ...
    if st_epochs > 0:
        ...
        netG.load_state_dict(torch.load(os.path.join(output_dir, f"netG_epoch_{st_epochs}.pth"), weights_only=True))
        netD.load_state_dict(torch.load(os.path.join(output_dir, f"netD_epoch_{st_epochs}.pth"), weights_only=True))
    ...
    optimizerD = optim.RMSprop(
        netD.parameters(), lr=lr, alpha=0.99, eps=1e-08, weight_decay=0, momentum=0
    )  # default parameters (some optimization here?)
    optimizerG = optim.RMSprop(
        netG.parameters(), lr=lr, alpha=0.99, eps=1e-08, weight_decay=0, momentum=0
    )  # default parameters (some optimization here?)

    res_path = os.path.join(output_dir, "training_results.csv")
    if st_epochs > 0:
    ...
    for epoch in range(st_epochs + 1, n_epochs):
        ...
        for i, data in tqdm(enumerate(train_loader, 0)):
            ...
            if i % 100 == 0 or i == len(train_loader) - 1:
                print(
                    "[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f Acc: %.4f"
                    % (
                        epoch,
                        n_epochs,
                        i,
                ...
                vutils.save_image(eval_imgs.data, "%s/fake_samples_epoch_%03d.png" % (output_dir, epoch))
