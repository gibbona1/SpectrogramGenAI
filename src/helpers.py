...
class MixDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        noise_wav, bird_wav, mix_wav = (
            rescale_wav(noise_wav),
            rescale_wav(bird_wav),
            rescale_wav(mix_wav),
        )
...
def adjust_model(model_name, num_classes, best=False, synth=0, knowledge_dist=False):
    ...
    if m_name == "resnet18":
        ...
        for name, param in model.named_parameters():
            if (
                name.startswith("features.25")
                or name.startswith("features.26")
                or name.startswith("features.28")
    ...
    if best:
        model.load_state_dict(
            torch.load(
                f'results/{model_name}_synth{synth}_raw_noind_in_{"kd_" if knowledge_dist else ""}best.pth',
                weights_only=True,
            )
        )
...
def load_ensemble(synth, num_classes, device, best=False, knowledge_dist=False):
    ...
    weights = [
        f'results/{mname}_synth{synth}_raw_noind_in_{"kd_" if knowledge_dist else ""}best.pth' for mname in mnames
    ]
    ...
    if best is True:
        ens_model.load_state_dict(
            torch.load(
                f'results/ensemble_synth{synth}_raw_noind_in_{"kd_" if knowledge_dist else ""}best.pth',
                weights_only=True,
            )
        )
    return ens_model
...
def get_neal_data(lab_file, test_path, classes):
    ...
    df_sub["formatted_file"] = df_sub.apply(
...
def eval_model(model, loader, criterion, device, noisered, n_channel, denoise_net, metrics):
    ...
    (
        ...
        f1_metric,
        accuracy_metric,
        top1_accuracy_metric,
        top5_accuracy_metric,
    ) = metrics
    ...
    with torch.no_grad():
        for inputs, labels in loader:
            ...
            if embs is not None:
                ...
                # Calculate the soft targets loss. Scaled by T**2
                distillation_loss = (
                    torch.sum(soft_targets * (soft_targets.log() - soft_prob)) / soft_prob.size()[0] * (temperature**2)
                )
...
def train_model(model, n_tuple, dfs, d_tuple, num_epochs=10, save_interval=10, batch_size=64):
    ...
    gen_df = pd.DataFrame(
        {
            "file_name": [os.path.join(gen_path, f) for f in os.listdir(gen_path)],
    ...
    if knowledge_dist:
        ...
        train_df_real = train_df_real.merge(
            train_df_emb,
            left_on=["file_name", "begin_time"],
            right_on=["file_name", "start_time"],
        )
        val_df = val_df.merge(
            train_df_emb,
            left_on=["file_name", "begin_time"],
        ...
        gen_df_emb["file_name"] = gen_df_emb["file_name"].apply(
            lambda x: x.replace(".birdnet.embeddings.txt.wav", ".png")
        )
    ...
    ext = (
        f"synth{str(synthetic)}"
        f'_{"nr" if noisered else "raw"}'
        f'_{"aind" if aindex else "noind"}'
        f'_{"mvn" if mvn else "in"}'
        f'{"_kd" if knowledge_dist else ""}'
    )
    ...
    if noisered:
        ...
        denoise_net.load_state_dict(torch.load(nr_path, weights_only=True))
    ...
    train_loader = DataLoader(
        dataset=SpecDatasetIm(train_df, im_path, pop_classes),
    ...
    for epoch in range(num_epochs):
        ...
        results_df.to_csv(
            os.path.join(output_dir, f"{model_name}_{ext}_training_results.csv"),
            index=False,
        )
...
def eval_best_model(model, n_tuple, dfs, d_tuple, batch_size=64, knowledge_dist=False):
    ...
    precision_metric = torchmetrics.Precision(
        task="multiclass", average="macro", num_classes=num_classes, zero_division=0
    ).to(device)
    recall_metric = torchmetrics.Recall(
        task="multiclass", average="macro", num_classes=num_classes, zero_division=0
    ).to(device)
    f1_metric = torchmetrics.F1Score(task="multiclass", average="macro", num_classes=num_classes, zero_division=0).to(
        device
    )
    accuracy_metric = torchmetrics.Accuracy(task="multiclass", num_classes=num_classes).to(device)
    top1_accuracy_metric = torchmetrics.Accuracy(task="multiclass", top_k=1, num_classes=num_classes).to(device)
    ...
    (
        val_loss,
        (val_accuracy, val_precision, val_recall, val_f1, val_top1, val_top3, val_top5),
        (val_y, val_preds),
    ) = eval_model2(
        model,
        val_loader,
...
def train_specunet(
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in tqdm(enumerate(train_loader, 0)):
            ...
            loss = criterion(
                sep_output,
                fast_resize_m1_1(torch.cat([noise_batch, bird_batch], dim=1).to(device)),
            )
            ...
            print(
                "[%d/%d][%d/%d] Loss: %.4f"  # | SDR: %.4f | FID: %.%4f' etc
        ...
        plot_sep(
            ...
            bird_batch,
            mix_batch,
            torch.unsqueeze(sep_output[:, 0, :], 1).cpu(),
            torch.unsqueeze(sep_output[:, 1, :], 1).cpu(),
            output_dir,
            epoch,
            save=True,
        )
        ...
        # Save results to the DataFrame
        if epoch % save_interval == 0:
            print("[%d/%d] TrainLoss:  %.4f | TestLoss: %.4f" % (epoch, num_epochs, train_loss, test_loss))
...
def plot_sep(
    ...
    # plt.show()
    if save:
        fig.savefig(os.path.join(output_dir, "sep_examples_%03d.png" % epoch))
