...
class SpecDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sav_name = None
        if self.sav_folder:
            sav_name = os.path.join(self.sav_folder, f'{row["file_name"]}_{int(row["begin_time"])}_{int(row["begin_time"])}.png')
...
loader = DataLoader(dataset=SpecDataset(df_filtered, pop_classes, sav_folder = sav_path), batch_size=batch_size, shuffle=False)
