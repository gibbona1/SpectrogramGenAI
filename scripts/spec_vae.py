...
from diff_modules import Encoder, VQEmbeddingEMA, Decoder, VQAE
...
def plot_images_torch(original_images, z, z_quantised, reconstructed_images, sav_folder, epoch):
    ...
    z_quantised_grid = np.concatenate([
        np.concatenate([z_quantised_np[:,0], z_quantised_np[:,1]], axis=1),  # Concatenate first two channels horizontally
        np.concatenate([z_quantised_np[:,2], z_quantised_np[:,3]], axis=1)   # Concatenate next two channels horizontally
    ], axis=-1)
...
#sve model
torch.save(model.state_dict(), os.path.join(model_sav_folder, f"ckpt.pt"))
torch.save(optimizer.state_dict(), os.path.join(model_sav_folder, f"optim.pt"))
print("Finish!!")
